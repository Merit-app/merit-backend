...
export async function getPublicProfile(username: string) {
  ...
  const verifiedHours = verifiedSessions.reduce(
    (sum, s) => sum + ((s.hours as number) ?? 0),
    0,
  );
  const uniqueOrgIds = new Set(
    allSessions.map((s) => s.org_id).filter(Boolean),
  );
  const lastActive =
...
export async function getProfileOrgs(username: string) {
  ...
  return (orgs ?? [])
    .map((org) => ({ ...org, ...(orgMap.get(org.id as string) ?? {}) }))
    .sort((a, b) => ((b as any).totalHours ?? 0) - ((a as any).totalHours ?? 0))
    .slice(0, 10);
}
