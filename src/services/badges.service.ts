...
async function getUserStats(userId: string): Promise<UserStats> {
  ...
  const dates = sessions.map((s) => new Date(s.date as string));
  const uniqueOrgIds = new Set(
    sessions.map((s) => s.org_id).filter(Boolean),
  );

  return {
    sessionCount: sessions.length,
    verifiedSessionCount: verified.length,
    verifiedHours: verified.reduce((sum, s) => sum + ((s.hours as number) ?? 0), 0),
    totalHours: sessions.reduce((sum, s) => sum + ((s.hours as number) ?? 0), 0),
...
> {
  ...
  const earnedMap = new Map<string, string>(
    (earnedResult.data ?? []).map((r) => [r.badge_id as string, r.earned_at as string]),
  );
