...
export function signPayload(payload: object, secret?: string, expiresInSec = 3600): string {
  const key = secret ?? env.MAGIC_LINK_SECRET;
  if (!key) throw new Error('MAGIC_LINK_SECRET is not configured. Set the env variable before using JWT operations.');
...
export function verifyPayload<T = Record<string, unknown>>(token: string, secret?: string): T | null {
  try {
    const key = secret ?? env.MAGIC_LINK_SECRET;
    if (!key) return null; // Can't verify without a key
